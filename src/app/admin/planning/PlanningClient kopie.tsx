...
// dragRef verwijderd - niet meer nodig
...
const handleBlockPointerMove = (event: React.PointerEvent<HTMLDivElement>) => {
    ...
    // Update ghost position directly (no state needed = smooth 60fps)
    if (ghostElement) {
      const newTop = event.clientY - offsetY

      // Snap to 15min intervals horizontally
    ...
    // Update current position for drop calculation (lightweight state update)
...
const handleBlockPointerUp = async (
    ...
    const { element, ghostElement, currentX, currentY, startX, startY } = dragState
    ...
    const ratio = Math.max(0, Math.min(1, (currentX - rect.left) / rect.width))
    const rawMinutes = viewStartMinutes + ratio * viewTotalMinutes
    const snappedMinutes = Math.round(rawMinutes / 15) * 15
