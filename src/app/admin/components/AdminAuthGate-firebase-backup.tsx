...
import { getFirebaseAuth, loginWithEmailPassword } from '@/lib/firebase-auth'
...
export default function AdminAuthGate({ children }: AdminAuthGateProps) {
  ...
  const handleBootstrap = async () => {
    ...
    try {
      ...
      const data = await response.json()
      if (!response.ok || !data.success) {
        throw new Error(data.error || 'Admin instellen mislukt')
      }
      setBootstrapRequired(false)
      setBootstrapError(null)
    } catch (err: any) {
      ...
      <div className="planning-modal-overlay">
        <div className="planning-modal">
          ...
          <p className="mt-2 text-sm text-slate-600">
            Meld je aan met je email en wachtwoord.
          </p>
          {error ? (
  ...
  if (bootstrapRequired) {
    return (
      <div className="planning-modal-overlay">
        <div className="planning-modal">
          <h2 className="text-xl font-semibold">Admin toegang aanvragen</h2>
          <p className="mt-2 text-sm text-slate-600">
            Je account is nog niet gekoppeld aan een admin-rol. Alleen geautoriseerde
            emails kunnen zichzelf als SYSTEM_ADMIN instellen.
          </p>
          {bootstrapError ? (
            <p className="mt-3 rounded-lg border border-red-200 bg-red-50 px-3 py-2 text-sm text-red-700">
              {bootstrapError}
          ...
          <div className="mt-4">
            ...
            >
              {bootstrapBusy ? 'Bezig...' : 'Maak mij SYSTEM_ADMIN'}
            </button>
          </div>
        </div>
      </div>
